...
class MailTexts {
  public static function getTexts() {
    return [
      ...
      'uk' => [
            'USER_SIGNUP' => [
                'Реєстрація {{to}} - OpenSupports',
                'Підтвердження вашого облікового запису',
                'Ласкаво просимо до нашого центру підтримки, {{name}}! Нам потрібно, щоб ви підтвердили цю електронну пошту, щоб отримати доступ до вашого облікового запису.',
                'Використайте цей код на {{url}}/verify-token/{{to}}/{{verificationToken}} або натисніть на кнопку нижче.',
            ],
            'USER_PASSWORD' => [
                'Пароль змінено - OpenSupports',
                'Пароль змінено',
                'Привіт, {{name}}. Ми хочемо повідомити, що ваш пароль було змінено через панель клієнта.'
            ],
            'USER_EMAIL' => [
                'Електронна пошта змінена - OpenSupports',
                'Електронна пошта змінена',
                'Привіт, {{name}}. Ми хочемо повідомити, що вашу електронну пошту було змінено на {{newemail}} через панель клієнта.'
            ],
            'PASSWORD_FORGOT' => [
                'Відновлення пароля - OpenSupports',
                'Відновлення пароля',
                'Привіт, {{name}}. Ви запросили відновлення пароля.',
                'Використайте цей код на {{url}}/recover-password?email={{to}}&token={{token}} або натисніть на кнопку нижче.',
            ],
            'USER_INVITE' => [
                'Вас запрошено - OpenSupports',
                'Вас запрошено',
                'Привіт, {{name}}. Вас запрошено приєднатися до нашого центру підтримки.',
                'Використайте цей код на {{url}}/recover-password?email={{to}}&token={{token}}&invited=true або натисніть на кнопку нижче, щоб налаштувати ваш пароль.'
            ],
            'USER_SYSTEM_DISABLED' => [
                'Зміна системи доступу - OpenSupports',
                'Зміна системи доступу',
                'Привіт, {{name}}. Система доступу до запитів була змінена.',
                'Ви можете отримати доступ до своїх запитів за допомогою електронної пошти та номера запиту. Натисніть на кнопку нижче, щоб переглянути свої запити.',
            ],
            'USER_SYSTEM_ENABLED' => [
                'Обліковий запис створено - OpenSupports',
                'Обліковий запис створено',
                'Привіт, {{name}}. Ми створили обліковий запис, за допомогою якого ви можете отримати доступ до запитів, які ви надіслали нам.',
                'Ви можете увійти до свого облікового запису, використовуючи цю електронну пошту <i>({{to}})</i> та пароль нижче. Будь ласка, змініть пароль, як тільки увійдете в систему.',
            ],
            'TICKET_CREATED' => [
                '#{{ticketNumber}} Запит створено - OpenSupports',
                'Запит створено',
                'Привіт, {{name}}. Ви надіслали новий запит з назвою <i>{{title}}</i> до нашого центру підтримки.',
                'Ви можете отримати доступ до запиту за його номером або натисніть на кнопку нижче.',
            ],
            'TICKET_RESPONDED' => [
                '#{{ticketNumber}} Нова відповідь - OpenSupports',
                'Запит відповів',
                'Привіт, {{name}}. Ви отримали відповідь на запит з назвою <i>{{title}}</i>.',
                'Будь ласка, натисніть нижче, щоб переглянути нову відповідь.'
            ],
            'TICKET_CLOSED' => [
                '#{{ticketNumber}} Запит закрито - OpenSupports',
                'Запит закрито',
                'Привіт, {{name}}. Запит, який ви надіслали, з назвою <i>{{title}}</i>, був закритий.',
                'Ви можете отримати доступ до запиту за його номером. Або натисніть на кнопку нижче.'
            ],
            'TICKET_CREATED_STAFF' => [
                '#{{ticketNumber}} Запит створено - OpenSupports',
                'Запит створено',
                'Користувач {{name}} створив новий запит з назвою <i>{{title}}</i>.',
                'Ви можете отримати доступ до запиту за його номером.',
            ],
        ],
    ];
  }
